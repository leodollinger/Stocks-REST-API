...
class Stock(Base):
    ...
    company_code = Column(String(15), unique=True)
    company_name = Column(String(30))

    stock_values_id = Column(Integer, ForeignKey('stock_values_table.id'), nullable=False, index=True)
    performance_data_id = Column(Integer, ForeignKey('performance_data_table.id'), nullable=False, index=True)
...
class Competitor(Base):
    ...
    market_cap_id = Column(Integer, ForeignKey('market_cap_table.id'), nullable=False, index=True)
